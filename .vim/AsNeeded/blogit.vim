            ...
            if vim.eval("exists('%s')" % var_name) == '1':
                ...
                try:
                    if var_name.endswith('password'):
                        vim.command('let ' + var_name + ' = inputsecret("Please input ' + var_name + ': ", "")')
                    else:
                        return None
                        #vim.command('let ' + var_name + ' = input("Please input ' + var_name + ': ", "")')
                    val = vim.eval('%s' % var_name)
                    if val == '':
                        vim.command('unlet ' + var_name)
                        return None 
                    else:
                        return val
                except vim.error:
